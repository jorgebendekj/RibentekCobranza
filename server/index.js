...
// ════════════════════════════════════════════════════════════════
// DEBUG — Webhook diagnostics (admin-auth required)
// ════════════════════════════════════════════════════════════════

/**
 * GET /api/debug/whatsapp-config
 * Returns the saved WhatsApp config for the current tenant (masked token).
 */
app.get('/api/debug/whatsapp-config', requireWorkspaceAdmin, async (req, res) => {
  const { tenantId } = req.workspaceAdmin;
  const { data, error } = await supabaseAdmin
    .from('whatsapp_configurations')
    .select('id, tenant_id, phone_number_id, waba_id, meta_id, verify_token, channel_name, token')
    .eq('tenant_id', tenantId)
    .is('deleted_at', null)
    .maybeSingle();
  if (error) return res.status(500).json({ error: error.message });
  if (!data) return res.json({ config: null, message: 'No WhatsApp config found for this tenant' });
  return res.json({
    config: {
      ...data,
      token: data.token ? `${data.token.slice(0, 6)}...${data.token.slice(-4)}` : null,
    },
  });
});

/**
 * POST /api/debug/simulate-incoming
 * Simulates an incoming WhatsApp message to test the full ingestion pipeline.
 * Body: { from_phone: "591XXXXXXXX", message_text: "hola test" }
 */
app.post('/api/debug/simulate-incoming', requireWorkspaceAdmin, async (req, res) => {
  const { tenantId } = req.workspaceAdmin;
  const { from_phone, message_text = 'Test message' } = req.body;
  if (!from_phone) return res.status(400).json({ error: 'from_phone is required' });

  try {
    // Get config to find phone_number_id
    ...
    if (!config) return res.status(400).json({ error: 'No WhatsApp config found for this tenant' });

    // Upsert contact
    let contactId;
    const { data: existingContact } = await supabaseAdmin
      .from('contacts').select('id').eq('tenant_id', tenantId).eq('phone_number', from_phone).is('deleted_at', null).maybeSingle();

    if (existingContact) {
      contactId = existingContact.id;
    } else {
      const { data: nc } = await supabaseAdmin
        .from('contacts').insert({ name: from_phone, phone_number: from_phone, tenant_id: tenantId }).select('id').single();
      contactId = nc?.id;
    }
    if (!contactId) return res.status(500).json({ error: 'Could not create/find contact' });

    // Upsert thread
    let threadId;
    const { data: existingThread } = await supabaseAdmin
      .from('whatsapp_threads').select('id').eq('contact_id', contactId).eq('tenant_id', tenantId).is('deleted_at', null).maybeSingle();

    if (existingThread) {
      threadId = existingThread.id;
    } else {
      const { data: nt } = await supabaseAdmin
        .from('whatsapp_threads').insert({ contact_id: contactId, tenant_id: tenantId, last_message: message_text, last_interaction: new Date().toISOString() }).select('id').single();
      threadId = nt?.id;
    }
    if (!threadId) return res.status(500).json({ error: 'Could not create/find thread' });

    // Insert message
    const { data: msg, error: msgErr } = await supabaseAdmin
      .from('whatsapp_messages').insert({ whatsapp_thread_id: threadId, message_text, incoming: true, read: false, sent_at: new Date().toISOString() }).select().single();
    if (msgErr) return res.status(500).json({ error: msgErr.message });

    // Update thread snapshot
    await supabaseAdmin.from('whatsapp_threads').update({ last_message: message_text, last_interaction: new Date().toISOString() }).eq('id', threadId);

    return res.json({ success: true, contact_id: contactId, thread_id: threadId, message: msg });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
});
